...
import { compressImage, uploadToOSS, validateFileSize, validateFileType } from '@/utils/oss';
...
export const useOSS = (options: UseOSSOptions = {}) => {
  ...
  const upload = useCallback(
    async (file: File, folder?: string) => {
      const { compress = true, onError, onProgress, onSuccess } = options;
      ...
      try {
        ...
        if (compress && file.type.startsWith('image/')) {
          try {
            fileToUpload = await compressImage(file, uploadConfig.compressQuality, uploadConfig.maxWidth);
          } catch (error) {
